...
export const useStaffState = () => {
  const { staff, setStaff, addStaffMember, updateStaffStatus, updateStaffInfo } = useStaffBasic(initialStaff);
  const { shifts, addShift } = useShiftManagement();
  const { updateStaffPerformance, updateStaffSchedule, updateCertifications } = usePerformanceManagement(staff, setStaff);
